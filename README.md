...
### 📌 포인트 적립 내역 기록을 이벤트 기반으로 분리한 이유

#### 책임 분리
- 포인트 적립은 사용자와 연관된 핵심 비즈니스 로직입니다. </br>
  하지만, 포인트 적립 내역은 감사/조회 등의 보조 기능입니다. </br>
  이 두 기능을 동일 트랜잭션으로 묶을 경우, 내역 저장 실패 시 전체 트랜잭션이 롤백되어 핵심 비즈니스 로직까지 영향이 갈 수 있는 점을 고려했습니다.

#### 장애 격리
- 적립 내역 저장은 `@TransactionalEventListener(phase = AFTER_COMMIT)`을 통해 메인 트랜잭션이 정상 커밋된 후에 동작합니다. </br>
이로 인해 적립 내역 저장 중 예외가 발생하더라도, 포인트 적립 자체는 영향을 받지 않습니다. </br>
사용자 경험과 데이터 무결성 측면을 고려하였습니다.

#### 트랜잭션 범위 최소화
- 포인트 적립 트랜잭션은 짧게 유지되며, 이후 로깅은 별도 트랜잭션에서 처리됩니다.

---

### 📌 바코드 발급 및 포인트 적립 로직 개선 (Redis 캐싱 도입)

이 프로젝트는 바코드 발급 후 포인트 적립이 바로 이어지는 사용 시나리오를 고려하여, Redis 캐싱을 도입해 시스템 성능을 최적화했습니다.

#### 1. 문제 인식: DB 부하와 지연 시간

기존 로직은 바코드가 발급될 때마다 회원의 존재 여부를 확인하기 위해 DB에 접근했습니다. 

하지만 포인트 적립 요청이 빈번하게 발생하면, DB에 불필요한 조회 부하가 집중되어 시스템 전체의 응답 속도가 느려질 수 있습니다.

- 기존 DB 조회 방식의 단점

|문제점|설명|
|---|---|
|DB 부하 증가|잦은 조회 요청으로 인해 DB 성능 저하|
|지연 시간|네트워크를 통해 DB에 접근하는 시간이 추가되어 응답 속도에 영향을 미칩니다.|

#### 2. 해결책: "발급 후 즉시 적립" 시나리오에 특화된 Redis 캐싱 적용

회원이 바코드를 발급받으면, 이 바코드가 곧바로 포인트 적립에 사용될 가능성이 높습니다. 

이 점을 활용하여, 바코드가 발급되는 즉시 Redis에 캐시를 저장하는 전략을 채택했습니다.

Redis 캐싱 로직
바코드 발급 시
- Barcode와 MemberId를 키-값 형태로 Redis에 즉시 저장합니다.
...
값(Value):
- BarcodeRedisValueDto(Long userId, String Barcode)
...
포인트 적립 요청
Redis 조회 (캐시 히트)
- 먼저 Redis에서 해당 바코드 정보를 조회합니다.
- 만약 Redis에 정보가 있다면, 바로 캐시에서 MemberId를 가져와 포인트 적립 로직을 진행합니다. </br> DB 조회 과정이 생략됩니다.

DB 조회 (캐시 미스)
- Redis에 정보가 없다면, DB에서 바코드를 조회하고 MemberId를 가져옵니다 </br> 이 경우, 다음 적립 요청을 위해 Redis에 캐시를 저장합니다.

#### 3. 기대 효과
성능 향상
- 대부분의 적립 요청이 Redis에서 처리되므로, DB 접근 횟수가 획기적으로 줄어들어 시스템 응답 속도가 빨라집니다.

DB 부하 감소
- DB는 꼭 필요한 경우(캐시 미스, 초기 적립)에만 사용되어, 안정적인 운영이 가능합니다.

사용자 경험 개선
- 빠른 응답 속도로 인해 포인트 적립 과정이 더 원활하고 빠르게 느껴집니다.
